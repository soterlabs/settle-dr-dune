...
--             (EOA-filtered recipient attribution) — FEBRUARY 2026 ONLY
-- =============================================================================
-- Same pipeline as diag_susds_ref1004_monthly_dr_eoa.sql, but instead of a
-- monthly rollup it aggregates PER WALLET over Feb 2026 (dt in [2026-02-01, 2026-03-01)):
...
--   * est_total_dr_usd      — estimated DR payout to the wallet for Feb 2026
...
with
    ...
    -- stops counting as 1004 afterwards (e.g. a Dec CowSwap buy clears an earlier
    -- Dec Paraswap-1004 tag, so the wallet is not counted as 1004 in Feb).
    cowswap_txs as (
        select distinct evt_tx_hash
    ...
    -- Eligible days only, restricted to February 2026
    complete_daily as (
...
-- =========================================================================
-- Per-wallet aggregation (February 2026), sorted by avg eligible balance
-- =========================================================================
select
    user_addr,
