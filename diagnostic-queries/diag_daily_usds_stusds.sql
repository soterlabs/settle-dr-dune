...
-- rationale). References EXACTLY ONE foundational TWA query (query_7877544).
--
-- Reports the USDS BASE the DR reward is applied to (NOT the reward):
--   sum_over_users(daily TWA stUSDS shares) x stUSDS->USDS rate (query_7877549).
--
...
from bal b
left join query_7877549 r on r.dt = b.dt
order by b.dt
