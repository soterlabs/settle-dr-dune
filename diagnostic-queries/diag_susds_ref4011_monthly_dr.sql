-- =============================================================================
-- Diagnostic: monthly DR rewards for sUSDS (Ethereum) ref_code 4011
-- =============================================================================
-- Methodology mirrors the main pipeline exactly:
--   TWA:        same two-tier attribution + TWA tail as twa_susds_susdc_erc4626.sql
--   Rate:       XR from rates_dr.sql  (0.6% APY 2024-2025 | 0.5% APY 2026+)
...
--
-- Only time segments where the forward-filled ref_code = 4011 contribute.
...
with
    ...
    -- =========================================================================
    -- Txs that emitted a 4011 referral event (used to find routed recipients)
    txs_4011 as (
    ...
    wallets_4011 as (
        ...
        -- sUSDS recipients in those txs (catches aggregator/router-routed deposits
        -- where the referral owner is the intermediary, not the final holder)
        select distinct tr."to" as user_addr
        from sky_ethereum.susds_evt_transfer tr
        cross join susds_addr s
        where tr.contract_address = s.addr
          and tr.evt_tx_hash in (select evt_tx_hash from txs_4011)
    ...
    latest_ref_per_tx as (
        ...
        from (
            ...
            from sky_ethereum.susds_evt_referral
            where owner in (select user_addr from wallets_4011)
        ) t
        where rn = 1
    ),
    ...
    raw_transfers as (
        ...
        where tr.contract_address = s.addr
          and tr."to" in (select user_addr from wallets_4011)
          and tr."to"   != 0x0000000000000000000000000000000000000000
          and tr."to"   != tr."from"
        ...
        left join ref_fallback      fb on tr.evt_tx_hash = fb.evt_tx_hash and lr.ref_code is null
        where tr.contract_address = s.addr
          and tr."from" in (select user_addr from wallets_4011)
    ...
    -- Intraday TWA segmented by ref_code (captures partial-day code transitions)
    daily_ref_segments as (
        ...
        where date(ts) = dt  -- exclude the synthetic start-of-day event from next day
        group by user_addr, dt, current_ref_code
    ),

    -- Date spine scoped to each user's active period
    ...
    -- Forward-fill balance and ref_code into every spine day.
    -- Transaction days with a 4011 segment use that segment's intraday TWA.
    -- Transaction days without a 4011 segment use 0 for that day's TWA.
    -- No-transaction days carry forward the last end-of-day balance + ref_code.
    complete_daily_all as (
        select
            ...
            case
                when seg.dt is not null
                    then seg.twa_balance          -- tx day, 4011 active portion
                when drs_any.dt is not null
                    then 0.0                      -- tx day, different code all day
                else
                    last_value(deb.end_of_day_balance) ignore nulls over (
                        partition by sp.user_addr
                        order by sp.dt
                        rows unbounded preceding
                    )                             -- no-tx day: carry forward
            end as twa_balance,
            coalesce(
        ...
        left join daily_ref_segments seg
           ...
           and seg.current_ref_code = 4011
        left join daily_end_balances deb
            on sp.user_addr = deb.user_addr and sp.dt = deb.dt
    ),

    -- Keep only days where wallet is in ref_code 4011 mode with positive balance
    complete_daily as (
        select user_addr, dt, twa_balance
        from complete_daily_all
        where forwarded_ref_code = 4011
          and twa_balance > 0
    ),
