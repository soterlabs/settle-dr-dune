...
``compute_twa`` processes one user at a time — a flat, small memory footprint
(the input is fully reconstructed per user; there is no full-dataset
materialization). It is validated byte-for-byte against Dune by the offline
fixtures.
...
def _compute_user(g: pd.DataFrame, fill_through: date) -> list[dict]:
    ...
    # --- intra-day segments per (dt, ref_code): [twab, seg_dur, seg_prod] -----
