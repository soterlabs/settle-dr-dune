-- =============================================================================
-- Template B: stUSDS per-user daily TWA (Ethereum)
-- -----------------------------------------------------------------------------
-- This is the canonical self-owned reference (Spark's query_5358161, see
-- raw-queries/query_5358161.txt) co-located here for completeness. It is the
-- template all other files follow: balance from ERC20 `Transfer`, ref_code from
-- the separate `Referral` event matched by (tx_hash, owner), forward-filled
-- (last-referral-wins).
--
-- The ONLY change vs. query_5358161 is the final SELECT, which adds the `symbol`
-- column so the output matches the shared schema used by the other twa_*.sql
-- files (and result_spark_s_usds_s_usdc_time_weighted_average_balance).
--
-- TABLE NAMES: sky_ethereum.stusds_evt_* are CONFIRMED (used by Spark directly).
-- PARAMETERS:
...
with
    token_targets (blockchain, token_symbol, token_addr, decimals, start_date) as (
        values
            ('ethereum', 'stUSDS', 0x99CD4Ec3f88A45940936F469E4bB72A2A701EEB9, 18, date '2024-09-01') -- start hardcoded to 2024-09-01 for uniformity; stUSDS itself launched 2025-08-25 so earlier days are empty.
    ),

    raw_referral_events as (
        select
            blockchain, evt_block_time as ts, evt_block_number, evt_tx_hash, evt_index,
        ...
        from (
            ...
            from sky_ethereum.stusds_evt_referral
        )
    ),

    latest_referral_per_tx as (
        select r.evt_tx_hash, r.user_addr, r.contract_address, r.blockchain, r.ref_code
        from (
            select r.evt_tx_hash, r.user_addr, r.contract_address, r.blockchain, r.ref_code,
                   row_number() over (
    ...
    raw_transfers_with_referral as (
        select
            'ethereum' as blockchain, tt.token_addr as contract_address,
            date(tr.evt_block_time) as dt, tr.evt_block_time as ts, tr.evt_block_number,
            tr.evt_tx_hash, tr.evt_index, tr."to" as user_addr,
            cast(tr.value as double) / power(10, tt.decimals) as amount_change,
            lr.ref_code, tt.token_symbol as symbol
        from sky_ethereum.stusds_evt_transfer tr
        ...
        left join latest_referral_per_tx lr
            ...
            and tr.contract_address = lr.contract_address and 'ethereum' = lr.blockchain
        ...
        union all
        select
            'ethereum', tt.token_addr, date(tr.evt_block_time), tr.evt_block_time, tr.evt_block_number,
            tr.evt_tx_hash, tr.evt_index, tr."from",
            -cast(tr.value as double) / power(10, tt.decimals),
            lr.ref_code, tt.token_symbol
        from sky_ethereum.stusds_evt_transfer tr
        join token_targets tt on tr.contract_address = tt.token_addr and tt.blockchain = 'ethereum'
        left join latest_referral_per_tx lr
