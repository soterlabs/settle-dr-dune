...
--             ** AMATSU-FAITHFUL VARIANT — owner-tagging, no recipient heuristic **
-- =============================================================================
-- This mirrors Amatsu's actual methodology (helpers.ts getUserKey / processEvent):
--   * The wallet credited with a referral is the address named in the Referral
--     event's `owner` field — NOTHING ELSE. Amatsu never tries to find the
--     "real" downstream recipient of a routed swap.
--   * For 1004 (Paraswap) every Referral.owner is a Paraswap router (pass-through).
--     We track that router's sUSDS balance like any other holder. Because the
--     router forwards what it receives, its time-weighted residual balance is
--     small (inventory/dust held across blocks) -> a few hundred $/month of DR.
--
-- WHY this avoids the protocol-contamination blow-up:
--   The previous variant (diag_susds_ref1004_monthly_dr.sql) added the NET sUSDS
--   recipient inside each 1004 tx as a "real user". When a user swaps via Paraswap
--   and deposits the sUSDS straight into Morpho / SparkLend in the same tx, the
--   PROTOCOL is the net recipient -> it got tagged 1004 and its entire multi-$100M
--   book was reallocated to 1004. Tagging only Referral.owner (the router) removes
--   that heuristic completely, exactly like Amatsu.
--
-- Everything below the wallet set (two-tier attribution, TWA tail, rate,
-- conversion, monthly rollup) is identical to diag_susds_ref4011_monthly_dr.sql.
--   Rate:       XR non-Spark (0.4% APY 2024-2025 | 0.5% APY 2026+)
--               1004 >= 1000 -> NON_SPARK_REF_RATE (Spark band is 100-999).
...
with
    ...
    -- =========================================================================
    -- 1. Wallets = referral OWNERS only (Amatsu owner-tagging). No recipient
    --    heuristic, no router exclusion -- we WANT to track the router's residual
    --    balance, which is what Amatsu credits to 1004.
    -- =========================================================================
    wallets_1004 as (
        select distinct owner as user_addr
        from sky_ethereum.susds_evt_referral
        where referral = 1004
    ),

    -- Tier 1: latest Referral per (tx, wallet) for tagged owners
