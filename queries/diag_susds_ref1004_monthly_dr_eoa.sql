...
--             ** EOA-ONLY RECIPIENT VARIANT — recipient attribution + contract filter **
-- =============================================================================
-- Why this variant exists:
--   * Owner-tagging (diag_susds_ref1004_monthly_dr_amatsu.sql) returns 0 — every
--     1004 Referral.owner is a Paraswap router, and the routers are perfect
--     pass-throughs (they hold ~0 sUSDS at all times). So crediting the owner
--     yields nothing, yet Amatsu reports ~$200-300/month.
--   * The non-zero number must come from the DOWNSTREAM recipients who actually
--     HOLD the sUSDS after the swap. The earlier recipient variant captured those,
--     but it also tagged DeFi protocols (Morpho singleton, SparkLend pool, pools,
--     bridges) when a user swapped and deposited into them in the same tx — and
--     then reallocated the protocol's entire multi-$100M book to 1004.
--
-- Fix (matches the conclusion that Amatsu filters protocols server-side):
--   Identify the net-positive sUSDS recipient inside each 1004 tx, then keep only
--   EXTERNALLY-OWNED ACCOUNTS by excluding every address Dune knows to be a
--   deployed contract (ethereum.contracts). This generically drops the routers,
--   Morpho, SparkLend, DEX pools, bridges, CEX vaults, etc. — no manual list —
--   leaving only real end-user wallets that genuinely hold the swapped sUSDS.
--
--   NOTE: this also drops legitimate smart-contract wallets (Safe/Argent). That
--   is the accepted trade-off for a no-maintenance filter; revisit if a material
--   Safe user is found to be missing.
--
-- TWA / rate / conversion / rollup are byte-for-byte the 4011 template.
--   Rate:       XR non-Spark (0.4% APY 2024-2025 | 0.5% APY 2026+); 1004 >= 1000.
--   Conversion: sUSDS shares -> USDS, inlined from conversion_susds.sql
--   Formula:    twa_shares / 365 x reward_per x conversion_rate = dr_usd/day
--
...
with
    ...
    -- recipient of the deposit is net-positive; routers and pass-through legs net
    -- to ~0 or negative.
    ...
    -- Keep only EOAs: drop every address Dune knows to be a deployed contract.
    -- This removes the Paraswap routers, Morpho, SparkLend, DEX pools, bridges,
    -- etc. in one shot — the source of the TWA blow-up — with no manual list.
    wallets_1004 as (
    ...
    -- Tier 2: fallback — last Referral in the tx (last-referral-wins across codes)
    ref_fallback as (
    ...
    -- =========================================================================
    -- 4. XR rate schedule — sUSDS non-Spark (0.4% APY 2024-2025 | 0.5% APY 2026+)
    -- =========================================================================
