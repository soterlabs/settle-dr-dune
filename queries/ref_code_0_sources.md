...
> that had it (`twa_susds_susdc_erc4626.sql`). In the current pipeline `0` is
> produced by exactly two mechanisms: a direct on-chain `Referral(referral = 0)`
> (Category A) and the PSM3 L2 `Swap.referralCode = 0` default (Category C).
...
- The monthly query keeps `0` as `0` (only `-999999 → 99`).
